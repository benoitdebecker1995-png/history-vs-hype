...
**Schedule:** Daily, 04:00 GMT-5 (09:00 UTC) — first run of the daily routine cap window.
...
STEP 6 — Always commit and open a PR (even if nothing interesting):
  BRANCH="claude/competitor-drop-$(date +%Y-%m-%d)"
  git checkout -b "$BRANCH"
  git add channel-data/competitor-drops/
  git commit -m "competitor scan $(date +%Y-%m-%d) — <one-line summary or 'no collisions'>"
  git push origin "$BRANCH"
  gh pr create --title "Competitor scan $(date +%Y-%m-%d)" --body "Auto-generated by Routine 1. Review analyst notes section if present." --base master

If nothing is interesting, still commit the raw report (no analyst notes section).
