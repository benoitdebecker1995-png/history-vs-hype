...
3. .claude/REFERENCE/VOICE-PROFILE.md — read the entire file. This is the single source of truth for voice match. The "Closings" section and the "Sentence rhythm" section are the most important.
...
- Compare the rough-cut closing audio to the locked thesis. If they don't align:
  ...
  - Audit candidates against VOICE-PROFILE.md. Surface honest verdicts (which are reject-on-voice, which are salvageable). If round 2 drifts academic, run Round 3 with sharper voice constraints. Save each round to its own file.
...
- Never silently fix voice issues. If a candidate violates VOICE-PROFILE.md, flag it explicitly and propose the rewrite. The user has explicitly rejected silent corrections.
...
- If the user pushes back on an audit verdict, defend with VOICE-PROFILE.md citations or notebook evidence. Do not flip on pushback alone.
...
- **Voice-honest:** Phase 3 mandates the audit against VOICE-PROFILE.md and explicitly rejects silent corrections. The Tripoli workflow burned three rounds of NotebookLM before getting voice right; this prompt encodes that lesson up-front.
