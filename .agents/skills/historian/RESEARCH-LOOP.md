...
- **Scope queries with `source_ids`** to force the right sources and avoid contamination from stray/off-topic uploads. **A mistyped source ID silently drops that source** — the answer then comes back grounded in whatever DID register (at #61 the silver chain came back grounded in the *khipu* sources because the Lane ID was wrong). Always check `sources_used` matches what you asked for.
- Large query outputs persist to a file — Read it.
- **⚠ `notebook_query` FABRICATES QUOTES — verify before any on-screen card (Rule 1).** Query synthesis will smooth a paraphrase into quotation marks and call it "character-exact." At #61 it invented Pagden "true owners"/"require tuition" (real terms: *dominium* / "poor and barbarous education") and "Roca Barea: 'a Protestant invention'" (a secondary article's paraphrase). **Protocol for every load-bearing quote/claim, run as a GATE before the script draft:**
  1. Check `sources_used`. Populated + the reference snippet contains the phrase = anchored. EMPTY `sources_used` (some sources do this every time — at #61: **Pagden** + **Roca Barea**) = NLM-asserted, NOT confirmed.
  2. Empty-sources_used or any doubt → **raw-grep**: `source_get_content` persists to a tool-results file (content is one giant line) → Python-search for the exact phrase. Present = real; absent = drop/reattribute.
  3. The returned snippet sometimes confirms the *source* but shows a *different passage* than the quote claimed (Gibson p.7 anchor) → source-confirmed-pending-PDF-eyeball, not card-ready.
  4. Page numbers from synthesis are unreliable → eyeball every page against the PDF before cards.
  5. **⚠ Adversarial CROSS-AUTHOR queries are the highest-risk for invention.** Asking NLM "does author A contradict author B / does A say X about B" will smooth in a fabricated scholar-on-scholar quote that reads perfectly real (at #59: invented Morris calling Karsh's work *"brazen mendacity and distortion"* — raw-grep found no such phrase in either Morris source). **Mandatory: raw-grep (`nlm source content <id>` → grep the phrase) every comparative/attribution quote before it's trusted, even when the surrounding facts check out.** Empty `sources_used` on a comparison query = treat the whole answer as hypotheses until grepped.

## Referee-format research process (the document-led engine — #61 Black Legend, 2026-06-18)
When the topic is a 2+-camp evidence-cropping conflict (Black/White Legend, partition narratives, treaty disputes), the loop above runs with these extra moves:
- **Modern consensus over dusty standards.** Don't anchor a contested beat on the old canonical (Hanke 1949) — acquire the recent synthesis (Pagden 1982 → Cervantes 2020 / Townsend 2019 / Restall 2018) and demote the dusty one to "the reading modern scholarship corrects." User: "that scholarship is still old… present the historical consensus as unbiased as possible."
- **Tier every framing to its named, dated proponent — and it's N camps, not 2.** Neutrality = "Roca Barea (2016) argues…", "Menéndez Pidal (1963) called…", never "some say" and never adjudicating the doctrine. Las Casas's numbers alone = a 5-camp spectrum. Let the DOCUMENT carry the verdict; VO stays plain, no moralizing.
- **Mechanism-grounding BEFORE writing (the Kurdistan rule, operationalized).** For every causal/legal/process beat, QUERY the notebook to understand the mechanism, write **MECHANISM NOTES** into the structure doc, and never conflate adjacent concepts (encomienda ≠ mita ≠ slavery; Vitoria's dominium vs his just-titles). Build a clarity device per beat (e.g. Lockhart's "Double Mistaken Identity").
- **The crop-test beat shape:** EVIDENCE (shared true fact) → CAMP-A crop (proponent) → CAMP-B crop (proponent) → THE DOCUMENT on screen → the QUALIFICATION both sides cut. The qualification IS the thesis.
- **Packaging stays Calm-Prosecutor:** keyword-anchored, two-sentence, **non-cringe** title (user guard); document-split thumbnail, no face; open on the document, save the modern culture-war for the close (Hipstorian led with the statue-war and flopped).
