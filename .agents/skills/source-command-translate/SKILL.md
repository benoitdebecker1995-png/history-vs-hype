...
# source-command-translate

Use this skill when the user asks to run the migrated source command `translate`.

## Command Template

# /translate - Document Translation Pipeline

Translate legal/historical documents clause-by-clause with cross-checking, legal annotations, and surprise detection. All LLM calls are made by Codex natively — no API key needed.
...
### Step 2: Translate Each Clause (Codex Native)
...
**2b. Translate using Codex natively** (model: Codex-sonnet-4-6):

Use the `system_prompt` and `user_prompt` from the payload to call Codex natively. This is a direct Codex LLM call — NOT a subprocess or API call.
...
**3c. Execute using Codex natively** (model: Codex-sonnet-4-6):

Use `payload['system_prompt']` and `payload['user_prompt']` to call Codex natively.
...
>
> Codex's translation: [text]
> Secondary translation: [text]
> Difference: [explanation]
>
> Options:
> 1. Accept Codex's translation and continue
...
**4b. Execute using Codex natively** (model: Codex-sonnet-4-6):
...
**5b. Execute using Codex natively** (model: Codex-sonnet-4-6):
...
|------|-------|-----------|
| Step 2: Translation | Codex-sonnet-4-6 | Nuanced legal/historical language requires quality |
| Step 3: Cross-check comparison | Codex-sonnet-4-6 | Semantic comparison accuracy |
| Step 4: Legal annotation | Codex-sonnet-4-6 | Legal terminology expertise |
| Step 5: Surprise detection | Codex-sonnet-4-6 | Contextual reasoning about narratives |
...
- All LLM calls are made by Codex natively — no ANTHROPIC_API_KEY needed
- One document at a time — no batch mode
- Cross-check uses DeepL/googletrans for an independent translation + Codex for semantic comparison
- The Python tools (`translator.py`, `formatter.py`, `structure_detector.py`) handle pure data processing only
- The `TranslationDataBuilder` class in `translator.py` builds prompts and parses responses; Codex executes the LLM calls
