...
| Task | Schedule | Wrapper (`.claude/routines/`) | Underlying command |
|---|---|---|---|
...
| HvH-GrowthRefresh (Routine 7) | daily 07:45 | `run-growth-refresh.ps1` | `python -m tools.youtube_analytics.growth_data --refresh` (pure python, no claude) — writes analytics.db |
| HvH-ChannelHealth (Routine 3) | 08:00 | `run-channel-health.ps1` | `claude -p channel-health-snapshot.md` — READS analytics.db, alert only on anomaly |
| HvH-Reconcile (Routine 6) | 08:30 | `run-reconcile.ps1` | `claude -p reconcile-daily.md` → `python -m tools.reconcile.reconcile --auto-publish-only` |
| HvH-StaleProjects (Routine 4) | 09:00 | `run-stale-projects.ps1` | `claude -p stale-project-nudge.md` — scans `_IN_PRODUCTION/` mtimes |
...
| HvH-BrainHygiene (Routine 5) | daily 22:00 | `run-brain-hygiene.ps1` | `claude -p brain-hygiene.md` — `.brain/_queue/` + index refresh |
...
`run-growth-refresh.ps1`, the live exemplar); claude-driven routines follow the checklist below. An
...
example (`claude --print` direct, `-RunLevel Highest`, no settings); it matches no live task.
...
### Adding a NEW claude-driven routine — checklist (F5, incident 2026-06-13)

All three or it will exit 0 having done nothing: (1) "▶ EXECUTION DIRECTIVE" at the top of the
routine `.md` (headless claude otherwise treats it as documentation and greets-and-exits);
(2) scoped `Edit(.brain/**)`/`Write(.brain/**)` allow-rules in `.claude/settings.local.json`
(already present — don't widen them); (3) wrapper logs full output + `$LASTEXITCODE` to
`.brain\_inbox\`.

### Cloud routines 1–2 — specs only, NOT scheduled

`competitor-drop-scanner.md` / `modern-relevance-hook-hunter.md` in `.claude/routines/` were
...
`.claude/settings.local.json` holds permissions only (incl. the `.brain/**` rules above) — no hooks.
...
**Health check for everything — LOOK-ONLY:** `claude mcp list` (run from the repo).
...
| vidiq | Enrichment ONLY, below the packaging gates (ADR-0012/0013); generation suite = REJECT per identity guard; scores never decide | Listed ✔ in `claude mcp list`; any cheap utility call (e.g. `vidiq_balance`) confirms auth | OAuth 2.0: `claude mcp add --transport http vidiq https://mcp.vidiq.com/mcp` then complete the browser OAuth (MUTATES config + token). Boost plan, ~5 credits/call, 10 for Video Watch. Down / out of credits → manual in-app VidIQ with paste-ready prompts. Setup: `docs/VIDIQ-MCP-SETUP.md`; tool map: `docs/VIDIQ-MCP-CAPABILITY-MAP.md`. |
| graphify-code | AST code graph (node count grows with commits — 63K at the 2026-07-02 rebuild; see rebuild log) — preferred over grep for structural questions (CLAUDE.md) | `mcp__graphify-code__graph_stats` | No auth. Freshness = post-commit hook (§2, race bug F11). Recovery/rollback table: `.claude/REFERENCE/GRAPHIFY-OPS.md`. |
...
| claude-in-chrome | Drives the user's real Chrome (extension-based; NOT in `.claude.json`) | `list_connected_browsers` | Permissions live in the Chrome extension, not this repo. |
...
| ⚠ Plaintext YouTube API key | `C:\Users\Benoi\.claude.json` under the STALE backslash project key (dead `youtube-data` server) | Known hygiene flag. Never print that file wholesale; when editing MCP config, target the forward-slash key. |
...
- **`.claude\scheduled_tasks.lock`** has zero repo references — best hypothesis is a Claude Code harness mutex. Don't build on it.
