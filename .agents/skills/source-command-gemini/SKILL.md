...
description: "Dispatches a bulk-read task to the Gemini CLI headless and captures the output to a file. Use when: the task is bulk reading — transcripts, long sources, competitor scans, multi-URL fetches — rather than judgment, so the reading stays out of context."
---

> **Codex note.** This is the Codex port of `.claude/commands/gemini.md`, which stays canonical.
