...
- **Per-file dict shims still exist** in `patterns.py` (`_report_to_patterns_dict`) and `feedback_parser.py` (`_report_to_feedback_dict`). These translate the typed `PostPublishReport` back into the two legacy dict shapes the historical callers expect (one with `avg_retention` as fraction + field `ctr_percent`, one with `avg_retention` as percent + field `ctr`). They are transitional. Deleting them requires migrating the ~6 remaining legacy-dict consumers (`analyze.py:248`, the report generators inside `patterns.py`, `backfill_all`'s internal loop) to consume `PostPublishReport` directly. **Action deferred** until friction motivates it.
